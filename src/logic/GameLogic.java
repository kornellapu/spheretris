...
import basic.Shape;
import indicator.PlacementIndicator;
...
public class GameLogic {
	 ...
	 * @return Késleltetés ms-ban*/
	public int getStepTime() {
		return millisBetweenSteps;
	}
	
	/**	A játéktér körüli nem látható mezõk nagyságát adja vissza (szélesség és magasság)
	 * 	Az új alakzatok ezekbe a mezõkre kerülnek és innen esnek be a játéktérre
	 * 
	 * 	@return A nem látható mezõk oszlopainak vagy sorainak száma (mindkét irányba ugyanakkora)
	 * */
	public int getOffScreenBorder() {
		return gameOffScreenBorder;
	}
	
	/**	Az alakzat jelölõnek a gettere
	 * @return Indikátor a leesõ alakzat elhelyezkedéséhez*/
	public PlacementIndicator getShapeIndicator() {
	 ...
	 * 
	 * @param shape Az alakzat amiz mozgatni akarunk
	 * @param pos Az adott pozícióba szeretnénk mozgatni az alakzat bal felsõ sarkát*/
	void moveShapeTo(Shape shape, Position pos) {
		...
		for(int wi = 0; wi < shape.getWidth(); wi++) {
			for(int hi = 0; hi < shape.getHeight(); hi++) {
				if(shape.getFullAt(wi, hi)) {
					gameField[pos.x + wi][pos.y + hi].setTo(shape.getBlocks()[wi][hi]);
				}
			}
		}
	 ...
	 * 
	 * @param shape Az alakzat amit szeretnénk elhelyezni
	 * @param dir Megadja, hogy a 8 irány közül melyiket szeretnénk kiválasztani (0: fent középen, 1: jobb felül, stb...)
	 * 
	 * @return Egy pozíciót, amiben az alakzat bal felsõ sarka kerülhet, hogy pontosan a kívánt helyen egyen a "spawn point"-on
	 * */
	 ...
	 * 
	 * @param distanceFromMiddle A középponttól vett gyûrû távolság
	 * 
	 * @return Logikai érték, hogy teljesen tele van vagy sem
	 * */
	 ...
	 * 	@param color Amilyen színre állítani kell
	 * */
	void changeColorInLoop(int distanceFromMiddle, Color color) {
		...
		for(int i = 0; i < length; i++) {
			gameField[LEFT_X  +i][TOP_Y   ].setColor( color );
			gameField[RIGHT_X   ][TOP_Y +i].setColor( color );
			gameField[RIGHT_X -i][BOT_Y   ].setColor( color );
			gameField[LEFT_X    ][BOT_Y -i].setColor( color );
		}
	}
	 ...
	 * 
	 * @param shape Alakzat, aminek a blokkjainak a távolságát nézzük 
	 * @param pos Amilyen pozícióhoz nézzük a távolságokat
	 * 
	 * @return Az összes olyan gyûrû távolság egy ArrayList-ben amiben az alakzat blokkjai megtalálhatóan a középponttól.
	 * */
	public ArrayList<Integer> getAllBlockDistanceIn(Shape shape, Position pos){
	 ...
	 * 
	 * 	@param distanceFromMiddle Középponttól vett gyûrû távolság, amin kívülrõl lesznek beljebb mozgatva a blokok
	 * */
	 ...
	 * 
	 * 	@param distanceFromMiddle A középponttól vett távolság amiben ki kell törölni a blokkokat
	 * */
	 ...
	 * 
	 * 	@param shape Az alakzat amiben az ellenõrzést el kell végezni, de csak itt
	 * */
	 ...
	 * 
	 * 	@param testPos Pozíció amit ellenõrizni kell
	 * 
	 ...
	 * 
	 * 	@return Játék vége logikai értéket adja vissza
	 * 	
	 * */
	 ...
	 * 
	 * 	@param shape Az alakzat aminek a következõ pozícióját ellenõrzi
	 * 	@param pos A megadott hely, ahol az alakzatra ellenõrizni kell
	 * 
	 * 	@return Következõ lépésre lesz-e érintkezés logikai érték
	 * */
		...
		for(int sxi = 0; !colliding && sxi < shape.getBlocksWidth(); sxi++) {
			for(int syi = 0; !colliding && syi < shape.getBlocksHeight(); syi++){
				...
				if( !shape.isPresentAt(oneBlockPos) && shape.getFullAt(sxi, syi) ){
					colliding = gameField[oneBlockPos.x][oneBlockPos.y].isFull();
				}
			}
		}
		return colliding;
	 ...
	 * 	
	 * 	@param field	A játéktér blokk mátrixa
	 * 	@param shape A törölni kívánt alakzat leírója
	 * */
	 ...
	 * */
	public boolean rotateFallingShape(){
		if(fallingShape == null)
			return false;
		...
		futureShape.rotate();
		for(int sxi = 0; sxi < futureShape.getWidth(); sxi++) {
			for(int syi = 0; syi < futureShape.getHeight(); syi++) {
	 ...
	 * 
	 * 	@param dir Az irány amibe mozgatnánk az alakzatot
	 * 
	 * 	@return Megtörtént-e a mozgatás
	 * */
	public boolean moveFallingShape(Dir dir) {
		//Ha nincs alakzat lépjen ki egybõl
		//VAGY
		//Ha vége a játéknak ne érzékelje a gombnyomásokat
		if(fallingShape == null || isGameOver())
			return false;
		
		//Ellenõrizni, hogy az irányvektorra merõlegesen mozgatja-e
		Position direction = new Position ( gravityPoint.x - (fallingShape.getPosition().x + fallingShape.getWidth()/2), gravityPoint.y - (fallingShape.getPosition().y + fallingShape.getHeight()/2) );
	 ...
	 * @param shape Az alakzat amire a gravitációt alkalmazuk
	 * @param gPoint Gravitációs pont
	 * 
	 * @return Alakzat következõ helye (bal felsõ sarok)
	 * */
	Position getNextPosByGravity(Shape shape, Position gPoint) {
		Position direction = new Position ( gPoint.x - (shape.getPosition().x + shape.getWidth()/2), gPoint.y - (shape.getPosition().y + shape.getHeight()/2) );
		direction.eightDirNormalize();
	 ...
	 * @param blockPos A blokk pozíciója amire a gravitációt alkalmazzuk
	 * @param gPoint Gravitációs pont
	 * 
	 * @return A blokk következõ helye
	 * */
	 ...
	 * 
	 * 	@param shape Az alakzat amit mozgatunk
	 * 	@param dir Az irány, hogy milyen irányba mozgatjuk
	 * 
	 ...
	 * 
	 * 	Megjegyzés: Ez az osztály a további egyedi formák egyszerû felvételét szolgálja a játéklogikába
	 * */
	 ...
	 * 
	 * 	@return Gravitációs indikátor
	 * */
	public TriangleIndicator getGravityIndicator() {
	 ...
	 * 	
	 * 	@param indic Az indikátor amit beállít
	 * */
	void setFallingGravityIndicator(TriangleIndicator indic) {
		...
		Position gravityVector = getGravityVector( new Position(fallingShape.getPos().x + fallingShape.getWidth()/2, fallingShape.getPos().y + fallingShape.getHeight()/2), gravityPoint);
		Position posToDraw = new Position(fallingShape.getPos());
		Dir dir  = Dir.UNKNOWN;
		...
		case -1:
			...
			case 0:
				//Balra
				posToDraw.x -= 1;
				posToDraw.y += fallingShape.getHeight()/2;
				dir = Dir.LEFT;
				break;
			case 1:
				//Balra lefelé
				posToDraw.x -= 1;
				posToDraw.y += fallingShape.getHeight();
				dir = Dir.LEFT_DOWN;
				break;
			}
			break;
		case 0:
			...
			case -1:
				//Felfelé
				posToDraw.x += fallingShape.getWidth()/2;
				posToDraw.y -= 1;
				dir = Dir.UP;
				break;
			case 1:
		...
		case 1:
			...
			case -1:
				//Jobbra felfelé
				posToDraw.x += fallingShape.getWidth();
				posToDraw.y -= 1;
				dir = Dir.RIGHT_UP;
				break;
			case 0:
	 ...
	 * 
	 * 	@param x X koordináta a blokk mátrixban
	 *  @param y Y koordináta a blokk mátrixban
	 * 
